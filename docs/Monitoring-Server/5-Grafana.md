...
- Sekarang kita lakukan instalasi grafananya
...
- Sekarang kita tinggal menghidupukan `grafana` kita 
...
- Karena kita pertama kali membuat `grafana` ini, sekarang kita akan menjalankan service dari si `prometheus`
...
- Karena kita telah mengedit konfigurasi `grafana` sekarang kita restart grafana kita
...
- Sekarang kita cek lagi status dari grana yang telah selesai kita lakukan instalasi
...
- Sekarang kita coba akses `grafana` kita, `grafana` berjalan di atas port:3000
...
- User dan pasword default dari grafana adalah `admin` `admin`
